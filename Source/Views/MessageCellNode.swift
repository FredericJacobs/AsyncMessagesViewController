...
class MessageCellNode: ASCellNode {
    ...
    private let avatarPlaceholder: ASLayoutSpec
    ...
    init(isOutgoing: Bool, topText: NSAttributedString?, contentTopText: NSAttributedString?, bottomText: NSAttributedString?, senderAvatarURL: NSURL?, senderAvatarImageSize: CGFloat, bubbleNode: ASDisplayNode) {
        ...
        avatarPlaceholder = ASRatioLayoutSpec.newWithRatio(1, child: avatarImageNode ?? ASLayoutSpec.new())
        avatarPlaceholder.flexBasis = ASRelativeDimensionMakeWithPoints(avatarImageSize)
    ...
    override func layoutSpecThatFits(constrainedSize: ASSizeRange) -> ASLayoutSpec! {
        return ASInsetLayoutSpec.newWithInsets(
            UIEdgeInsetsMake(1, 4, 1, 4),
            child: ASStackLayoutSpec.newWithStyle(
                ...
                children: Array.filterNils([
                    ...
                    contentTopTextNode == nil
                        ? nil
                        : ASInsetLayoutSpec.newWithInsets(
                            UIEdgeInsetsMake(0, 22 + avatarImageSize, 0, 0),
                            child: contentTopTextNode),
                    ASStackLayoutSpec.newWithStyle(
                        ASStackLayoutSpecStyle(
                            direction: .Horizontal,
                            spacing: 2,
                            justifyContent: .Start, // Never used
                            alignItems: .End),
                        children: isOutgoing ? [bubbleNode, avatarPlaceholder] : [avatarPlaceholder, bubbleNode]),
                    bottomTextNode])))
    }
